...

{
    ...
    float *udists, *uspikes1, *uspikes2;
    ...
    for(trac1 = 0; trac1 < *num_trains; ++trac1)
        for(trac2 = 0; trac2 < *num_trains;  ++trac2)
            if (trac1 != trac2) {
                ...
                uspikes1 = (float *)mxGetPr(uspikes1Pr);
                uspikes2 = (float *)mxGetPr(uspikes2Pr);
                
                udistsPr = mxCreateNumericArray(0, 0, mxSINGLE_CLASS, mxREAL);
                mxSetM(udistsPr, 1);
                mxSetN(udistsPr, num_uspikes[trac1]);
                mxSetData(udistsPr, mxMalloc(sizeof(float) * num_uspikes[trac1]));
