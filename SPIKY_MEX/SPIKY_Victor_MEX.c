...

{
    ...
    float *tli, *tlj, *cost_array, *vdv, *scr, diff;
    mxArray *scrPr;
    
    tli = (float *)mxGetPr(tli_in);
    tlj = (float *)mxGetPr(tlj_in);
    cost_array = (float *)mxGetPr(cost_array_in);  
    
    num_cost_array = mxGetNumberOfElements(cost_array_in);

    vdv_out = mxCreateNumericArray(0, 0, mxSINGLE_CLASS, mxREAL);
    mxSetM(vdv_out, 1);
    mxSetN(vdv_out, num_cost_array);
    mxSetData(vdv_out, mxMalloc(sizeof(float)*num_cost_array));
    vdv = (float *)mxGetPr(vdv_out);
    ...
    scrPr = mxCreateNumericArray(0, 0, mxSINGLE_CLASS, mxREAL);
    mxSetM(scrPr, 2);
    mxSetN(scrPr, minNsp + 1);
    mxSetData(scrPr, mxMalloc(sizeof(float) * 2 * (minNsp + 1)));
    scr = (float *)mxGetPr(scrPr);
