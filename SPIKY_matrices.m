...
if f_para.num_instants>0
    m_res.movie_vects=zeros(m_para.num_sel_bi_measures,f_para.num_select_pairs,f_para.num_instants,'single');
end
...
if f_para.group_matrices && d_para.num_select_train_groups>1 && d_para.num_select_train_groups<f_para.num_trains
    if f_para.num_instants>0
        m_res.group_movie_mat=zeros(m_para.num_sel_bi_measures,f_para.num_select_train_groups,f_para.num_select_train_groups,f_para.num_instants,'single');
    ...
    if f_para.num_selective_averages>0
        m_res.group_movie_mat_sa=zeros(m_para.num_sel_bi_measures,f_para.num_select_train_groups,f_para.num_select_train_groups,f_para.num_selective_averages,'single');
        for sgc=1:f_para.num_select_train_groups
            for sgc2=sgc:f_para.num_select_train_groups
                if sgc~=sgc2
...
if comparison && r_para.num_runs==1 
    ...
    m_res.movie_mat=zeros(m_para.num_sel_bi_measures,f_para.num_frames,f_para.num_trains,f_para.num_trains,'single');
...
if f_para.num_frames>0
    ...
    for mac=1:m_para.num_sel_bi_measures
        ...
        if isfield(m_res,'movie_vects_sa')
            ...
            for sc=1:f_para.num_selective_averages
                ...
                plot_mat=plot_mat+plot_mat';
                movie_mat(sc,:,:)=plot_mat;
            end
            eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),...
                '.selective_averages=squeeze(movie_mat);'])
        end
        if isfield(m_res,'movie_vects_ta')
            ...
            for tc=1:f_para.num_triggered_averages
                ...
                plot_mat=plot_mat+plot_mat';
                movie_mat(tc,:,:)=plot_mat;
            end
            eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),...
                '.triggered_averages=squeeze(movie_mat);'])
        end
        if isfield(m_res,'group_movie_mat') && f_para.num_instants>0
            eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),...
        ...
        if isfield(m_res,'group_movie_mat_ta')
            eval(['results.',char(m_para.all_measures_string(m_para.select_bi_measures(mac))),...
    ...
    if comparison
        if h_para.dendrograms
            h_para.max_col=h_para.num_all_matrices;
        elseif h_para.group_matrices
            h_para.max_col=h_para.num_reg_matrices;
        elseif h_para.num_measures>1 && h_para.num_frames>1
            h_para.max_col=h_para.num_frames;
        else
            h_para.max_col=4;
        end
    ...
    elseif sequence
        if ruc==r_para.num_runs   % ruc==1 || (ruc==r_para.num_runs && f_para.num_instants==0)
            if f_para.dendrograms+f_para.group_matrices>0
                ...
                h_para.max_cols=[1 2 3 4 3 3 4 4];
            end
    ...
    h_para.supos=zeros(h_para.num_all_subplots,4);
    for spc=1:h_para.num_all_subplots
        if h_para.num_all_subplots==1
            h_para.supos(spc,1:4)=[0.3  0.05   0.4*ones(1,2)];
        elseif h_para.num_all_subplots==2
            if colbar==0
                h_para.supos(spc,1:4)=[0.09+0.46*(h_para.cols(spc)-1)  0.06   0.38*ones(1,2)];
            elseif colbar==1
                h_para.supos(spc,1:4)=[0.08+0.44*(h_para.cols(spc)-1)  0.09   0.35*ones(1,2)];
            else
                h_para.supos(spc,1:4)=[0.075+0.47*(h_para.cols(spc)-1)  0.1   0.33*ones(1,2)];
            end
        elseif h_para.num_all_subplots==3
            if colbar==0
                h_para.supos(spc,1:4)=[0.075+0.3*(h_para.cols(spc)-1)  0.18   0.25*ones(1,2)];
            elseif colbar==1
                h_para.supos(spc,1:4)=[0.075+0.28*(h_para.cols(spc)-1)  0.18   0.24*ones(1,2)];
            else
                h_para.supos(spc,1:4)=[0.075+0.3*(h_para.cols(spc)-1)  0.22   0.19*ones(1,2)];
            end
        elseif h_para.num_all_subplots==4
            if h_para.max_col==4
                if colbar==0
                    h_para.supos(spc,1:4)=[0.075+0.23*(h_para.cols(spc)-1)  0.24   0.18*ones(1,2)];
                elseif colbar==1
                    h_para.supos(spc,1:4)=[0.075+0.22*(h_para.cols(spc)-1)  0.24   0.17*ones(1,2)];
                else
                    h_para.supos(spc,1:4)=[0.075+0.23*(h_para.cols(spc)-1)  0.28   0.13*ones(1,2)];
                end
            else
                if f_para.group_matrices
                    if colbar<2
                        h_para.supos(spc,1:4)=[0.2+0.4*(h_para.cols(spc)-1)  0.05+0.25*(h_para.rows(spc)<max(h_para.rows))   0.2*ones(1,2)];
                    else
                        h_para.supos(spc,1:4)=[0.18+0.41*(h_para.cols(spc)-1)  0.05+0.25*(h_para.rows(spc)<max(h_para.rows))   0.2*ones(1,2)];
                    end
                else
                    if colbar<2
                        h_para.supos(spc,1:4)=[0.2+0.4*(h_para.cols(spc)-1)  0.05+0.25*(h_para.rows(spc)<max(h_para.rows))   0.21*ones(1,2)];
                    else
                        h_para.supos(spc,1:4)=[0.18+0.41*(h_para.cols(spc)-1)  0.05+0.25*(h_para.rows(spc)<max(h_para.rows))   0.21*ones(1,2)];
                    end
                end
            end
        elseif h_para.num_all_subplots==5
            if colbar<2
                h_para.supos(spc,1:4)=[0.11+0.3*(h_para.cols(spc)-1)+0.14*(h_para.rows(spc)-1)  0.05+0.23*(h_para.rows(spc)<max(h_para.rows))   0.205*ones(1,2)];
            else
                h_para.supos(spc,1:4)=[0.08+0.31*(h_para.cols(spc)-1)+0.14*(h_para.rows(spc)-1)  0.07+0.22*(h_para.rows(spc)<max(h_para.rows))   0.19*ones(1,2)];
            end
        elseif h_para.num_all_subplots==6
            if colbar==0
                h_para.supos(spc,1:4)=[0.11+0.3*(h_para.cols(spc)-1)  0.05+0.23*(h_para.rows(spc)<max(h_para.rows))   0.205*ones(1,2)];
            elseif colbar==1
                h_para.supos(spc,1:4)=[0.1+0.28*(h_para.cols(spc)-1)  0.05+0.23*(h_para.rows(spc)<max(h_para.rows))   0.205*ones(1,2)];
            else
                h_para.supos(spc,1:4)=[0.08+0.31*(h_para.cols(spc)-1)  0.08+0.22*(h_para.rows(spc)<max(h_para.rows))   0.19*ones(1,2)];
            end
        elseif h_para.num_all_subplots==7
            if colbar<2
                h_para.supos(spc,1:4)=[0.075+0.23*(h_para.cols(spc)-1)+0.1*(h_para.rows(spc)-1)  0.05+0.22*(h_para.rows(spc)<max(h_para.rows))   0.18*ones(1,2)];
            else
                h_para.supos(spc,1:4)=[0.075+0.23*(h_para.cols(spc)-1)+0.1*(h_para.rows(spc)-1)  0.13+0.18*(h_para.rows(spc)<max(h_para.rows))   0.13*ones(1,2)];
            end
        elseif h_para.num_all_subplots==8
            if colbar==0
                h_para.supos(spc,1:4)=[0.075+0.23*(h_para.cols(spc)-1)  0.05+0.23*(h_para.rows(spc)<max(h_para.rows))   0.18*ones(1,2)];
            elseif colbar==1
                h_para.supos(spc,1:4)=[0.075+0.22*(h_para.cols(spc)-1)  0.07+0.21*(h_para.rows(spc)<max(h_para.rows))   0.17*ones(1,2)];
            else
                h_para.supos(spc,1:4)=[0.075+0.23*(h_para.cols(spc)-1)  0.13+0.18*(h_para.rows(spc)<max(h_para.rows))   0.13*ones(1,2)];
            end
        end
    end
