...

{
    ...
    #define run_pico_starts_ruc_in prhs[8]
    #define ints_in prhs[9]
    #define udists_in prhs[10]
    
    int *num_pairs, *run_time_lengths_ruc, *run_pico_starts_ruc, *num_trains, *prev_indy, *foll_indy, *isi_indy, pac = 0, sac, trac1, trac2, M;
    float *bi_spike_diffs_t, *udists, *udists2, *folli, *previ, *ints;
    ...
    run_pico_starts_ruc = (int *)mxGetPr(run_pico_starts_ruc_in);
    ints = (float *)mxGetPr(ints_in);
    
    bi_spike_diffs_t_out = mxCreateNumericArray(0, 0, mxSINGLE_CLASS, mxREAL);
    mxSetM(bi_spike_diffs_t_out, *num_pairs);
    mxSetN(bi_spike_diffs_t_out, *run_time_lengths_ruc);
    mxSetData(bi_spike_diffs_t_out, mxMalloc(sizeof(float) * *num_pairs * *run_time_lengths_ruc));
    bi_spike_diffs_t = (float *)mxGetPr(bi_spike_diffs_t_out);
    ...
    for(trac1 = 0; trac1 < *num_trains-1; ++trac1)
        for(trac2 = trac1 + 1; trac2 < *num_trains;  ++trac2) {
            ...
            for(sac = 0; sac < *run_time_lengths_ruc; ++sac)
                ...
                /ints[trac1 + *num_trains*(isi_indy[sac] - *run_pico_starts_ruc)]*ints[trac2 + *num_trains*(isi_indy[sac] - *run_pico_starts_ruc)]
                ...
                /ints[trac2 + *num_trains*(isi_indy[sac] - *run_pico_starts_ruc)]*ints[trac1 + *num_trains*(isi_indy[sac] - *run_pico_starts_ruc)])
                /((ints[trac1 + *num_trains*(isi_indy[sac] - *run_pico_starts_ruc)] + ints[trac2 + *num_trains*(isi_indy[sac] - *run_pico_starts_ruc)])
                *(ints[trac1 + *num_trains*(isi_indy[sac] - *run_pico_starts_ruc)] + ints[trac2 + *num_trains*(isi_indy[sac] - *run_pico_starts_ruc)])/2);
        }
    return;
}