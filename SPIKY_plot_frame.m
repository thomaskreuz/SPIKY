...
if frc<=f_para.num_instants
    ...
    if ~any(mov_handles.mov_line_lh)
        ...
        if f_para.x_realtime_mode==0
            ...
            xval=d_para.tmax;
            mov_handles.mov_line_lh(3)=line(xval*ones(1,2),sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+[0.15 1.15]/1.3*f_para.subplot_size(2),...
                'Visible',p_para.mov_vis,'Color',p_para.mov_col,'LineStyle',':','LineWidth',p_para.mov_lw,'UIContextMenu',mov_handles.mov_line_cmenu);
        end
        mov_handles.mov_line_lh(1)=line(xval*ones(1,2),sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+[0 0.15]/1.3*f_para.subplot_size(2),...
        ...
        mov_handles.mov_line_lh(2)=line(xval*ones(1,2),sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+[1.15 1.3]/1.3*f_para.subplot_size(2),...
            'Visible',p_para.mov_vis,'Color',p_para.mov_col,'LineStyle',p_para.mov_ls,'LineWidth',p_para.mov_lw,'UIContextMenu',mov_handles.mov_line_cmenu);
    ...
    if frc<=f_para.num_instants+f_para.num_selective_averages*f_para.num_average_frames
        ...
        if savc2==1
            ...
            for selc=1:f_para.num_sel_ave(savc)
                mov_handles.selave_line_lh(1,selc)=line(f_para.selective_averages{savc}(2*selc-1:2*selc),...
                    (sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+0.075/1.3*f_para.subplot_size(2))*ones(1,2),...
                ...
                mov_handles.selave_line_lh(2,selc)=line(f_para.selective_averages{savc}(2*selc-1:2*selc),...
                    (sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+1.225/1.3*f_para.subplot_size(2))*ones(1,2),...
                    'Visible',p_para.selave_vis,'Color',p_para.selave_col,'LineStyle',p_para.selave_ls,...
                    'LineWidth',p_para.selave_lw,'UIContextMenu',mov_handles.selave_line_cmenu);
            end
        ...
        if ~any(mov_handles.trigave_plot_lh)
            ...
            mov_handles.trigave_plot_lh(1)=plot(f_para.triggered_averages{tavc},(sum(f_para.subplot_size(...
                f_para.singles))-f_para.subplot_start(2)+0.075/1.3*f_para.subplot_size(2))*ones(1,length(f_para.triggered_averages{tavc})),...
            ...
            mov_handles.trigave_plot_lh(2)=plot(f_para.triggered_averages{tavc},(sum(f_para.subplot_size(...
                f_para.singles))-f_para.subplot_start(2)+1.225/1.3*f_para.subplot_size(2))*ones(1,length(f_para.triggered_averages{tavc})),...
            ...
            set(trigaver_plot_lh(1),'XData',f_para.triggered_averages{tavc},'YData',...
                (sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+0.075/1.3*...
            ...
            set(trigaver_plot_lh(2),'XData',f_para.triggered_averages{tavc},'YData',...
                (sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+1.225/1.3*...
                f_para.subplot_size(2))*ones(1,length(f_para.triggered_averages{tavc})))
        end
    end
end
...
mat_title_fh=zeros(h_para.num_all_matrices,1+f_para.dendrograms);
mat_label_cmenu = uicontextmenu;
mat_label_fh=zeros(h_para.num_all_matrices,2*(1+f_para.dendrograms));
if f_para.num_select_train_groups>1
    mat_sgs_cmenu = uicontextmenu;
    mat_sgs_lh=zeros(h_para.num_all_matrices,f_para.num_select_train_groups-1,2);
end
mat_thick_sep_cmenu = uicontextmenu;
mat_thick_sep_lh=zeros(h_para.num_all_matrices,length(d_para.thick_separators),2);
mat_thin_sep_cmenu = uicontextmenu;
mat_thin_sep_lh=zeros(h_para.num_all_matrices,length(d_para.thin_separators),2);
...
for matc=1:h_para.num_all_matrices
    ...
    if matc<=h_para.num_measures
        if frc<=f_para.num_instants
            plot_vect=squeeze(m_res.movie_vects(matc,:,mi));
        ...
        if matc==1
            results.matrices=zeros(h_para.num_measures,f_para.num_trains,f_para.num_trains);
        end
        results.matrices(matc,:,:)=plot_mat;
    else
        if frc<=f_para.num_instants
            plot_mat=squeeze(m_res.group_movie_mat(matc-h_para.num_measures,:,:,mi));
        elseif frc<=f_para.num_instants+f_para.num_selective_averages*f_para.num_average_frames
            if savc2==1
        ...
        if matc==h_para.num_measures+1
            results.group_matrices=zeros(h_para.num_measures,f_para.num_select_train_groups,f_para.num_select_train_groups);
        end
        results.group_matrices(matc-h_para.num_measures,:,:)=plot_mat;
    end
    ...
    if h_para.num_all_subplots<4
        mat_title_fh(matc,1)=title(m_res.bi_mat_str{matc},...
    ...
    if h_para.rows(matc)==max(h_para.rows) || f_para.dendrograms
        if matc<=h_para.num_measures
    ...
    if h_para.cols(matc)==1 || max(h_para.cols)<=2
        if matc<=h_para.num_measures
    ...
    if f_para.dendrograms==1
        ...
        if ~any(any(isnan(plot_mat))) && size(mm,1)>1
            ...
            if matc<=h_para.num_measures
                ...
                if matc==h_para.num_measures+1
                    results.group_dendros=cell(1,h_para.num_measures);
                end
            ...
            if f_para.spike_train_color_coding_mode>1
                ...
                if matc>h_para.num_measures || (f_para.spike_train_color_coding_mode==2 && f_para.num_select_train_groups>1)
                    ...
                    if matc<=h_para.num_measures
                        dh_para.cols=cm(dcol_indy(d_para.select_train_groups(f_para.select_group_vect)),:);
                    else
                        dh_para.cols=cm(dcol_indy(d_para.select_train_groups),:);
                    end
                else
