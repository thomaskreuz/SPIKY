...

{
    ...
    bi_isi_ratio_out = mxCreateNumericArray(0, 0, mxSINGLE_CLASS, mxREAL);
    mxSetM(bi_isi_ratio_out, *num_pairs);
    mxSetN(bi_isi_ratio_out, *run_pico_lengths_ruc);
    mxSetData( bi_isi_ratio_out, mxMalloc(sizeof(float) * *num_pairs * *run_pico_lengths_ruc));
    bi_isi_ratio = (float *)mxGetPr(bi_isi_ratio_out);
