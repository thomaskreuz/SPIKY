...
for trac=1:d_para.num_trains
    ...
    if any(dummy1)
        ...
        spikes{trac}=unique(round(dummy2/d_para.dts)*d_para.dts);
...
if d_para.num_select_train_groups>1
    ...
    group_names_fh(1)=text(xl(1)-0.075*(xl(2)-xl(1)),yl(1)+0.7*(yl(2)-yl(1)),'Spike',...
        'Color',p_para.group_names_col,'FontSize',p_para.group_names_fs,'FontWeight',p_para.group_names_fw,'FontAngle',p_para.group_names_fa,'UIContextMenu',group_names_cmenu);
    group_names_fh(2)=text(xl(1)-0.075*(xl(2)-xl(1)),yl(1)+0.35*(yl(2)-yl(1)),'trains',...
        'Color',p_para.group_names_col,'FontSize',p_para.group_names_fs,'FontWeight',p_para.group_names_fw,'FontAngle',p_para.group_names_fa,'UIContextMenu',group_names_cmenu);
    fh_str='group_names';
    SPIKY_handle_font
...
if d_para.num_total_spikes<d_para.max_total_spikes
    ...
    xl=xlim; yl=ylim;
    image_mh=imagesc(spike_density,'XData',[s_para.itmin+s_para.itrange/(2*f_para.psth_num_bins) s_para.itmax-s_para.itrange/(2*f_para.psth_num_bins)],...
        'YData',[0.05+1/(2*d_para.num_trains) 1.05-1/(2*d_para.num_trains)]);
...
setappdata(handles.figure1,'image_mh',image_mh)
