...
for frc=1:h_para.num_frames
    for matc=1:h_para.num_measures*(1+f_para.group_matrices)
        ...
        mat_sph(spc)=subplot('position',h_para.supos(spc,:),'UIContextMenu',mat_cmenu);
        ...
        %end
        %if matc==h_para.num_measures
        if h_para.rows(spc)==max(h_para.rows) || h_para.dendrograms
            if matc<=h_para.num_measures
        ...
        if h_para.num_all_subplots<=4 && (h_para.cols(spc)==1 || (f_para.group_matrices && (h_para.bm_order==1 || frc==1)))
            if matc<=h_para.num_measures
                mat_label_fh(spc,2)=ylabel('Spike trains',...
                    'Visible',p_para.mat_label_vis,'Color',p_para.mat_label_col,'FontSize',p_para.mat_label_fs,...
                    'FontWeight',p_para.mat_label_fw,'FontAngle',p_para.mat_label_fa,'UIContextMenu',mat_label_cmenu);
            else
                mat_label_fh(spc,2)=ylabel('Spike train groups',...
                    'Visible',p_para.mat_label_vis,'Color',p_para.mat_label_col,'FontSize',p_para.mat_label_fs,...
                    'FontWeight',p_para.mat_label_fw,'FontAngle',p_para.mat_label_fa,'UIContextMenu',mat_label_cmenu);
            end
        end
        ...
        if matc<=h_para.num_measures % || max(h_para.rows)==1
            ...
            if h_para.num_all_matrices<4
                mat_title_fh(spc,2)=title(title_str,...
        ...
        if f_para.num_select_train_groups>1
            for sgc=1:f_para.num_select_train_groups-1
                mat_sgs_lh(spc,sgc,1)=line(xl,(0.5+f_para.num_trains-f_para.cum_num_select_group_trains(sgc))*ones(1,2),...
                ...
                mat_sgs_lh(spc,sgc,2)=line((0.5+f_para.cum_num_select_group_trains(sgc))*ones(1,2),yl,...
                    'Visible',p_para.mat_sgs_vis,'Color',p_para.mat_sgs_col,'LineStyle',p_para.mat_sgs_ls,...
                    'LineWidth',p_para.mat_sgs_lw,'UIContextMenu',mat_sgs_cmenu);
            end
        end
        if isfield(d_para,'thick_separators') && ~isempty(d_para.thick_separators)
            for sec=1:length(d_para.thick_separators)
                mat_thick_sep_lh(spc,sec,1)=line(xl,(0.5+f_para.num_trains-d_para.thick_separators(sec))*ones(1,2),...
                ...
                mat_thick_sep_lh(spc,sec,2)=line((0.5+d_para.thick_separators(sec))*ones(1,2),yl,...
                    'Visible',p_para.mat_thick_sep_vis,'Color',p_para.mat_thick_sep_col,'LineStyle',p_para.mat_thick_sep_ls,...
                    'LineWidth',p_para.mat_thick_sep_lw,'UIContextMenu',mat_thick_sep_cmenu);
            end
        end
        if isfield(d_para,'thin_separators') && ~isempty(d_para.thin_separators)
            for sec=1:length(d_para.thin_separators)
                mat_thin_sep_lh(spc,sec,1)=line(xl,(0.5+f_para.num_trains-d_para.thin_separators(sec))*ones(1,2),...
                ...
                mat_thin_sep_lh(spc,sec,2)=line((0.5+d_para.thin_separators(sec))*ones(1,2),yl,...
                    'Visible',p_para.mat_thin_sep_vis,'Color',p_para.mat_thin_sep_col,'LineStyle',p_para.mat_thin_sep_ls,...
                    'LineWidth',p_para.mat_thin_sep_lw,'UIContextMenu',mat_thin_sep_cmenu);
            end
        end
        ...
        if h_para.dendrograms==1
            fig=figure(f_para.num_fig);
            dendros_sph(spc)=subplot('position',h_para.supos(h_para.num_all_matrices+spc,1:4),'UIContextMenu',dendros_cmenu);
            if matc<=h_para.num_measures
