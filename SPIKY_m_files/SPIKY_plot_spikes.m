...
f_para.singles=zeros(1,s_para.num_subplots);
for suc=1:s_para.num_subplots
    f_para.singles(suc)=find(f_para.subplot_posi==suc,1,'first');                               % new subplot
...
f_para.subplot_size(f_para.singles)=subplot_size2;
...
f_para.subplot_start(f_para.singles)=f_para.subplot_start2;
...
sp_bounds_lh(1)=line(xl,(sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+0.15/1.3*f_para.subplot_size(2))*ones(1,2),...
    'Color',p_para.sp_bounds_col,'LineStyle',p_para.sp_bounds_ls,'LineWidth',p_para.sp_bounds_lw,'UIContextMenu',sp_bounds_cmenu);
sp_bounds_lh(2)=line(xl,(sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+1.15/1.3*f_para.subplot_size(2))*ones(1,2),...
    'Color',p_para.sp_bounds_col,'LineStyle',p_para.sp_bounds_ls,'LineWidth',p_para.sp_bounds_lw,'UIContextMenu',sp_bounds_cmenu);
lh_str='sp_bounds';
SPIKY_handle_line
...
if f_para.num_select_train_groups>1 && length(find(diff(f_para.select_group_vect)))==f_para.num_select_train_groups-1
    ...
    for sgc=1:f_para.num_select_train_groups
        if sgc<f_para.num_select_train_groups
            sgs_lh(sgc)=line([s_para.itmin s_para.itmax],(sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(1.15-f_para.cum_num_select_group_trains(sgc)/f_para.num_trains)/1.3*f_para.subplot_size(2))*ones(1,2),...
                'Color',p_para.sgs_col,'LineStyle',p_para.sgs_ls,'LineWidth',p_para.sgs_lw,'UIContextMenu',sgs_cmenu);
        end
        group_names_fh(sgc)=text(xl(1)-0.06*(xl(2)-xl(1)),sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(1.15-f_para.select_group_center(sgc)/f_para.num_trains)/1.3*f_para.subplot_size(2),f_para.select_group_names{sgc},...
    ...
    fh_str='group_names';
    SPIKY_handle_font
    set(gca,'YTick',fliplr(sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(1.15-f_para.cum_num_select_group_trains/f_para.num_trains)/1.3*f_para.subplot_size(2)),'YTickLabel',fliplr(f_para.cum_num_select_group_trains))
else
    group_names_cmenu = uicontextmenu;
    ...
    if mod(f_para.num_trains,2)==0
        set(gca,'YTick',sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(0.15+[0 0.5])/1.3*f_para.subplot_size(2),'YTickLabel',[f_para.num_trains f_para.num_trains/2])
    else
        set(gca,'YTick',sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(0.15+[0 1-(f_para.num_trains-1)/2/f_para.num_trains])/1.3*f_para.subplot_size(2),'YTickLabel',[f_para.num_trains (f_para.num_trains-1)/2])
    end
end
...
if isfield(d_para,'thick_separators') && ~isempty(d_para.thick_separators)
    ...
    for sec=1:length(f_para.thick_separators)
        thick_sep_lh(sec)=line([s_para.itmin s_para.itmax],(sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(1.15-f_para.thick_separators(sec)/f_para.num_trains)/1.3*f_para.subplot_size(2))*ones(1,2),...
...
if isfield(d_para,'thin_separators') && ~isempty(d_para.thin_separators)
    ...
    for sec=1:length(f_para.thin_separators)
        thin_sep_lh(sec)=line([s_para.itmin s_para.itmax],(sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(1.15-f_para.thin_separators(sec)/f_para.num_trains)/1.3*f_para.subplot_size(2))*ones(1,2),...
...
if isfield(d_para,'thick_markers') && ~isempty(d_para.thick_markers)
    ...
    for mac=1:length(d_para.thick_markers)
        thick_mar_lh(mac)=line(d_para.thick_markers(mac)*ones(1,2),sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+[0.15 1.15]/1.3*f_para.subplot_size(2),...
...
if isfield(d_para,'thin_markers') && ~isempty(d_para.thin_markers)
    ...
    for mac=1:length(d_para.thin_markers)
        thin_mar_lh(mac)=line(d_para.thin_markers(mac)*ones(1,2),sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+[0.15 1.15]/1.3*f_para.subplot_size(2),...
    ...
    if mod(f_para.spike_col,4)>1
        ...
        for trac=1:f_para.num_trains
            colpat_ph(trac,1)=patch([f_para.tmin f_para.tmin-0.01*(f_para.tmax-f_para.tmin)*ones(1,2) f_para.tmin],...
                sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(1.15-(trac-1+[0 0 1 1])/f_para.num_trains)/1.3*f_para.subplot_size(2),...
...
if f_para.num_total_spikes<f_para.max_total_spikes
    ...
    xl=xlim; yl=ylim;
    imagesc(spike_density,'XData',[s_para.itmin+s_para.itrange/(2*f_para.psth_num_bins) s_para.itmax-s_para.itrange/(2*f_para.psth_num_bins)],...
        'YData',sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+[(0.15+1/(2*f_para.num_trains))/1.3*f_para.subplot_size(2) ...
        (1.15-1/(2*f_para.num_trains))/1.3*f_para.subplot_size(2)])
    if f_para.num_trains<30 && f_para.psth_num_bins<30
        for trac=1:d_para.num_trains-1
            line([s_para.itmin s_para.itmax],(sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(0.15+...
        ...
        for bc=1:f_para.psth_num_bins-1
            line(s_para.itmin+bc*s_para.itrange/f_para.psth_num_bins*ones(1,2),...
                sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+[0.15/1.3*f_para.subplot_size(2) 1.15/1.3*f_para.subplot_size(2)],'Color','k','LineStyle',':')
        end
    end
end
...
if f_para.histogram
    ...
    set(hh,'XData',sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(0.15+(f_para.num_trains-1-((1:f_para.num_trains)-1)+0.5)/f_para.num_trains)/1.3*f_para.subplot_size(2))
    ...
    line((s_para.itmin+1.22*s_para.itrange)*ones(1,2),sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+(0.15+(f_para.num_trains-1-(([0 f_para.num_trains])-1))/f_para.num_trains)/1.3*f_para.subplot_size(2),'LineStyle',':','Color','k')
    %text(1.18*s_para.itrange,sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+0.1,num2str(max(f_para.num_pspikes)),'FontSize',12)
    hist_max_cmenu = uicontextmenu;
    hist_max_fh=text((s_para.itmin+1.18*s_para.itrange),sum(f_para.subplot_size(f_para.singles))-f_para.subplot_start(2)+0.05/1.3*f_para.subplot_size(2),num2str(max(f_para.num_pspikes)),'Color',p_para.hist_max_col,'FontSize',p_para.hist_max_fs,...
