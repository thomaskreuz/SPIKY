...
csrear = SPIKY_vanRossum_rearrange(single(st1), single(st2), int32(length(pooledst))); % N + M %rearanging indices
...
FFF = SPIKY_vanRossum_sort(single(st1), single(st2), single(sum1), single(sum2), int32(length(pooledst))); % N + M %sorting values
