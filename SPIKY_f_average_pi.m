...
% This includes piecewise constant (such as ISI) which should first be transformed via the function
...
for trac=1:num_trains
    x_all(sum(num_sup(1:trac-1))+(1:num_sup(trac)))=x3(trac,1:num_sup(trac));
    if any(x3(trac,2:2:num_sup(trac)-2)==x3(trac,3:2:num_sup(trac)-1))
        if num_sup(trac)>2
            while any(x3(trac,2:2:num_sup(trac)-2)==x3(trac,3:2:num_sup(trac)-1))
                x3(trac,2:2:num_sup(trac)-2)=x3(trac,2:2:num_sup(trac)-2)-dts/2;
                x3(trac,3:2:num_sup(trac)-1)=x3(trac,3:2:num_sup(trac)-1)+dts/2;
            end
        end
    end
end

[x_ave,ind1,ind2]=unique(x_all);
indy=zeros(size(x));
for trac=1:num_trains
...
x_ave=x_ave(sort([1 2:end-1 2:end-1 end]));
...
for trac=1:num_trains
    ...
    y_all(trac,dindy)=interp1(x3(trac,1:num_sup(trac)),y(trac,1:num_sup(trac)),x_ave(dindy));
end
y_ave=mean(y_all,1);
